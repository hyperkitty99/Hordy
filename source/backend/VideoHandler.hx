...
class VideoHandler extends vlc.bitmap.VlcBitmap
{
	...
	function update()
	{
		if ((Controls.instance.ACCEPT || Controls.instance.PAUSE) && isPlaying) finishVideo();
	...
	function onVLCError() throw "VLC caught an error!";
...
class VideoHandler extends hxvlc.openfl.Video {
	...
	@:noCompletion override function update(_):Void {
		if (!events.contains(true)) return;
		...
		if (events[5]) {
			...
			errmsg != null && errmsg.length > 0 ? onEncounteredError.dispatch(errmsg) : onEncounteredError.dispatch('Unknown error');
		}
			...
			if (mustRecreate) {
				try {
					if (ClientPrefs.data.cacheOnGPU && Lib.current.stage != null && Lib.current.stage.context3D != null) {
						...
						if (ClientPrefs.data.cacheOnGPU) Log.warn('Unable to utilize GPU texture, resorting to CPU-based image rendering.');
		...
		if (events[14]) {
			...
			if (__renderable && planes != null) {
				try {
					...
					else if (bitmapData != null && bitmapData.image != null) bitmapData.setPixels(bitmapData.rect, planesBytes);
				}
				catch (e:haxe.Exception)
