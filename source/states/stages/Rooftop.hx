...
class Rooftop extends BaseStage {
	...
	override function create() {
		...
		add(speakers2 = new BGSprite("bgs/lisa/speakers", 1760, -2180, ['speakers'], null, 1.1, null, true));
		...
		add(speaker = new BGSprite("bgs/hordy/Kolonki", 2350, 850, ['kolonka'], null, 1.3, null, true));
	...
	override function createPost() for (thing in [whiteShit, croud_dash, boyfriendGroup]) PlayState.instance.insert(members.indexOf(game.dadGroup) + 1, PlayState.instance.remove(thing));
	...
	override function beatHit() curBeat >= 224 ? for (sprite in [speakers, speakers2, speakerF]) sprite.dance() : for (sprite in [speakers, speakers2, speakerF, croud_dash, speaker2, speaker]) sprite.dance();
}