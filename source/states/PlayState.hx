...
class PlayState extends MusicBeatState
{
	...
	public static var boyfriendGhost:Character;
	...
	public var blockControls:Bool = false;
	...
	override public function create()
	{
		...
		if (stageData.introSoundsSuffix != null)
			introSoundsSuffix = stageData.introSoundsSuffix;
		...
		if(stageData.cam_intensity != null)
			cameraIntensity = stageData.cam_intensity;
		...
		if (stageData.start_camera_pos == null) 
			stageData.start_camera_pos = [0, 0];
		...
		switch (curStage)
		{
			...
			case 'hordy': new states.stages.Hordy();
			case 'hordyfire': new states.stages.HordyFire();
		...
		camFollow.setPosition(stageData.start_camera_pos[0], stageData.start_camera_pos[1]);
		...
		if (!seenCutscene && isStoryMode) {
			switch (SONG.song.toLowerCase()) {
				case "ungrowing": startVideo('ungrowing');
				case "mind blown": startVideo('mind-blown');
				case "completed": startVideo('completed');
				default: 
					startCallback();
					endVideo();
			}
		} else if (!seenCutscene && !isStoryMode && SONG.song.toLowerCase() == 'lock in and explode')
			startVideo('sigma');
		else 
			startCallback();
		...
		songTxt = new FlxText(20, 190, 0, curSong + '\n\nBy ${SONG.credit}').setFormat(Paths.font("vcr.ttf"), 20, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		songTxt.borderSize = 1.25;

		bgbar = new AttachedSprite();
		bgbar.makeGraphic(Std.int(songTxt.width + 32), Std.int(songTxt.height + 22), 0x80000000);
		bgbar.sprTracker = songTxt;
		bgbar.yAdd = bgbar.xAdd = -12;
		add(bgbar);
		add(songTxt);

		songTxt.x = -bgbar.width - 20;
		...
		stagesFunc(function(stage:BaseStage) stage.actualCreatePost());

		songTxt.camera = bgbar.camera = camOther;
	}
	...
	public function startVideo(name:String) {
		...
		video.bitmap.onEndReached.add(endVideo, true);
	...
	inline function startAndEnd() endingSong ? endSong() : startCountdown();
	...
	public function startCountdown()
	{
		...
		startTimer = new FlxTimer().start(Conductor.crochet / 1000 / playbackRate, function(tmr:FlxTimer)
		{
			...
			switch (swagCounter)
			{
				...
				case 4:
					tick = START;
					FlxTween.tween(songTxt, {x: 12}, 1, {ease: FlxEase.quartInOut, onComplete: _ -> {
						FlxTimer.wait(2.5, () -> {FlxTween.tween(songTxt, {x: -bgbar.width - 20}, 1, {ease: FlxEase.quartInOut, onComplete: _ -> {
							songTxt.destroy(); bgbar.destroy();
						}});});
					}});
			}
	...
	override function closeSubState()
	{
		...
		if (paused)
		{
			...
			paused = false;
			if (SONG.song.toLowerCase() == 'poydem viydem') persistentUpdate = true;
		}
	}
	...
	override public function update(elapsed:Float)
	{
		...
		if(useDirectionalCamera && !inCutscene && !paused) {
			if(focusedCharacter != null && !focusedCharacter.isAnimationNull()) {
				if(focusedCharacter.getAnimationName().startsWith('sing')) {
					switch (focusedCharacter.getAnimationName().substring(4)) {
						case 'UP' | 'UP-alt':
							targetY -= cameraIntensity / cameraSpeed / playbackRate;
						case 'DOWN' | 'DOWN-alt':
							targetY += cameraIntensity / cameraSpeed / playbackRate;
						case 'LEFT' | 'LEFT-alt':
							targetX -= cameraIntensity / cameraSpeed / playbackRate;
		...
		if (controls.PAUSE && startedCountdown && canPause && !blockControls)
		{
			openPauseMenu();
		}

		if ((controls.PAUSE || controls.ACCEPT) && playingVideo && (startingSong || endingSong)) endVideo();

		if(!endingSong && !inCutscene && allowDebugKeys && !blockControls)
		{
				...
				if(notes.length > 0)
				{
					if(startedCountdown)
					{
						...
						notes.forEachAlive(function(daNote:Note)
						{
							...
							if(daNote.isSustainNote && strum.sustainReduce && (!daNote.mustPress ? (!daNote.gfNote ? !dad.stunned : true) : true)) daNote.clipToStrumNote(strum);
	...
	inline public function checkIcon() {
	...
	public function moveCamera(isDad:Bool)
	{
		...
		stagesFunc(function(stage:BaseStage) stage.moveCamera(isDad));
	}
	...
	public function playTVideo(name:String) {
	...
	private function onKeyPress(event:KeyboardEvent):Void
	{
		...
		if (!controls.controllerMode)
		{
			...
			if(FlxG.keys.checkStatus(eventKey, JUST_PRESSED) && !blockControls) keyPressed(key);
		}
	}
	...
	function opponentNoteHit(note:Note):Void {
		...
		if (!note.gfNote ? !char.stunned : true) {
			...
			var charGhost:Character = dadGhost;
			if(note.gfNote) charGhost = gfGhost;
	...
	public function goodNoteHit(note:Note):Void
	{
		...
		if(!note.noAnimation) {
			...
			if(char != null)
			{
				...
				if (note.noteType == 'Dynamite Note') {
	...
	override function destroy() {
		...
		BF_POS = flixel.util.FlxDestroyUtil.put(BF_POS);
		GF_POS = flixel.util.FlxDestroyUtil.put(GF_POS);
		DAD_POS = flixel.util.FlxDestroyUtil.put(DAD_POS);
		...
		var dynamicArray:Array<Dynamic> = [instance, inst, boyfriendGroup, dadGroup, gfGroup, dad, gf, boyfriend, notes, camFollow, camFollowPos, strumLineNotes, 
			opponentStrums, playerStrums, grpNoteSplashes, comboGroup, uiGroup, healthBar, timeBar, iconP1, iconP2, camHUD, camGame, camOther, scoreTxt, timeTxt, 
			startCallback, endCallback, video];

		for (bullshits in dynamicArray) bullshits = null; //kill me
		...
		inline function __clearArray<T>(a:Array<T>):Array<T> {
			if (a != null) while (a.length != 0) a.pop();
			return null;
		}
	...
	@:noCompletion inline function get_playingVideo():Bool return video?.bitmap.isPlaying ?? false;
}
