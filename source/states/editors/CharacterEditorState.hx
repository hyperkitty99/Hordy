...
#else
@:bitmap("assets/images/editors/cursorCross.png")
...
class CharacterEditorState extends MusicBeatState
{
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>) {
		...
		else if(sender is FlxUINumericStepper)
		{
			if (sender == scaleStepper)
			{
				...
				if(ghost != null) {
					ghost.scale.set(character.jsonScale, character.jsonScale);
					ghost.updateHitbox();
				}
	...
	override function update(elapsed:Float)
	{
		...
		if(FlxG.keys.justPressed.R && !FlxG.keys.pressed.CONTROL) FlxG.camera.zoom = 1;
			...
			FlxG.camera.zoom -= elapsed * FlxG.camera.zoom * shiftMult * ctrlMult * 2;
			if(FlxG.camera.zoom < 0.1) FlxG.camera.zoom = 0.1;
		}

		if (FlxG.mouse.wheel != 0)
		{
			FlxG.camera.zoom += elapsed * FlxG.mouse.wheel * FlxG.camera.zoom * shiftMult * ctrlMult * 2;
			if(FlxG.camera.zoom < 0.1) FlxG.camera.zoom = 0.1;
			if(FlxG.camera.zoom > 3) FlxG.camera.zoom = 3;
		}
		...
		if((FlxG.mouse.pressed || FlxG.mouse.pressedMiddle) && (FlxG.mouse.deltaScreenX != 0 || FlxG.mouse.deltaScreenY != 0))
		{
			FlxG.camera.scroll.x -= FlxG.mouse.deltaScreenX / 1.75;
			FlxG.camera.scroll.y -= FlxG.mouse.deltaScreenY / 1.75;
		}
