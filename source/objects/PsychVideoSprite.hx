...
class PsychVideoSprite extends hxvlc.flixel.FlxVideoSprite {
    ...
    public function addCallback(vidCallBack:String,func:Void->Void) {
        switch (vidCallBack) {
            case 'onEnd': if (func != null) bitmap.onEndReached.add(func);
            case 'onStart': if (func != null) bitmap.onOpening.add(func);
            case 'onFormat': if (func != null) bitmap.onFormatSetup.add(func);
        }
    }
    ...
    inline public static function globalPause() for (i in heldVideos) i.pause();
    inline public static function globalResume() for (i in heldVideos) i.resume();
}
